...
public class Application {
    ...
    public static void main(String[] args) throws Exception{
        ...
        while (option != 5) {
            ...
            switch (option) {
                ...
                case 4:
                    // No implementado...
                    //counter = undo(counter, stack); //own implementation, not exception covered
                    try {
    ...
    /*
    public static int undo(int counter, ArrayList<Integer> stack) throws Exception{
        int n = stack.size();

        int opc = stack.remove(n-1);
        if (opc == 1){
            return counter - 1;
        } else if (opc == 2) {
            return counter / 2;
